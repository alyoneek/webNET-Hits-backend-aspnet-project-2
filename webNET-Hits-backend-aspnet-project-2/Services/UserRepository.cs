...
namespace webNET_Hits_backend_aspnet_project_2.Services
{
    public class UserRepository<T> : IEfRepository<T> where T : class //BaseEntity
    {
        ...
        public async Task EditRange(IEnumerable<T> newEntitiesRange)
        {
            _context.Set<T>().UpdateRange(newEntitiesRange);
            await _context.SaveChangesAsync();
        }
        ...
        public async Task DeleteRange(IEnumerable<T> removeEntitiesRange)
        {
            _context.Set<T>().RemoveRange(removeEntitiesRange);
            await _context.SaveChangesAsync();
        }
    }
}
